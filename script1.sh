...
cat $ip_addr_get| sort -nr | head >> scriptresults.txt # Sort top 10 ip addresses and save result to text file
printf "\n" >> scriptresults.txt
...
cat pdfraw5.tmp | sort -nr | head >> scriptresults.txt #Sort the top 10 pdf downloads from highest to lowest and save result to text file
printf "\n" >> scriptresults.txt
...
cat get_browsers_uniq.tmp | sort -nr | head -n 5 >> scriptresults.txt # Sort top 5 web browsers used and save result to text file
printf "\n" >> scriptresults.txt
...
cat ipv6raw2.tmp | sort -nr |head >> scriptresults.txt #Save result to text file
